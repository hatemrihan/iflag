...
const [isPresent, safeRemove] = usePresence();
    ...
    >
        ...
        <motion.div
        className="absolute h-full bg-stone-800"initial={{
          width: '100%'
        }}
        animate={{ width:0}}
        exit={{ width: '100%'}}
        ...
        <cite className="mt-4 md:mt-8 not-italic block md:text-lg text-2xl lg:text-xl" ref={citeScope}>{name}, {role}  {company}
        </cite>
        </blockquote>
        </div>);
